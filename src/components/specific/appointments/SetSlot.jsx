  ...
  return (
    <div className="setslot-main-container">
      <div className="setslot-header">
        <h1 onClick={() => setView(view === 'slots' ? 'beds' : 'slots')} className="setslot-clickable-header">Set Slots</h1>
        <div className="setslot-divider" />
        <h1 onClick={() => setView(view === 'slots' ? 'beds' : 'slots')} className="setslot-clickable-header">
          Beds
        </h1>
      </div>
      ...
      {view === 'slots' ? (
        <>
          <label htmlFor="averageTime">Average Time For OP</label>
          <input
            type="text"
            name="averageTime"
            id="averageTime"
          ...
          <div className="setslot-slots-container">
            ...
            <div className="setslot-slots">
              {timeSlots.slice(0, 7).map((time, index) => (
                <button
                  key={index}
                  className={`setslot-slot-button ${selectedSlot === time ? 'selected' : ''}`}
